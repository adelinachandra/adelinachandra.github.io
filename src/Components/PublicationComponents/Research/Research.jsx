            ...
            {data.map((item, i) => (
                <ol key={i} className=''>
                    <li className='ms-4 py-2'>
                        ...
                        { (item.year    !== "" && item.type !== "") ? (<p id="research-informations">{item.year} <span>&#183;</span> in <span className='italic'>{item.type}</span></p>) : ("")}
                        { (item.author  !== "") ? (<p id="research-author" className='mb-1'> {BoldText(item.author, 'Adelina Chandra')} </p>) : ("")}
                    </li>
                </ol>
            ))}
        </section>
